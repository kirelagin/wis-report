...
def build(bld):
    bld(source='Introduction.pd Index.pd Highlight.pd '
        'Search.pd Infrastructure.pd Conclusion.pd', bibliography='bib.bib')

    bld.add_group()

    bld(features='tex', type='pdflatex', source='kirill.latex', target='kirill.pdf')
    bld(features='tex', type='pdflatex', source='nikita.latex', target='nikita.pdf')
